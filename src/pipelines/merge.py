...
def prepare_data(resnet_path, text_and_bert_path, attributes_path, data_path, catboost=False, train=False):
    # Read parquets
    ...
    print("Merging...")
    # Merge resnet
    full_data = data.merge(resnet, left_on="variantid1", right_on="variantid", how="left")
    ...
    # Embed
    print("Loading embedder and tokenizer...")
    ...
    print("Embedding...")
    print("     attributes_1...")
    full_data = bert_64_transformer(model, tokenizer, "attributes_1").transform(full_data)
    print("     categories_1...")
    full_data = bert_64_transformer(model, tokenizer, "categories_1").transform(full_data)
    print("     attributes_2...")
    full_data = bert_64_transformer(model, tokenizer, "attributes_2").transform(full_data)
    print("     categories_2...")
    full_data = bert_64_transformer(model, tokenizer, "categories_2").transform(full_data)
    ...
    if train:
        full_data = full_data[["variantid1", "variantid2", "concated_embeddings_1", "concated_embeddings_2", "target"]]
    else:
        full_data = full_data[["variantid1", "variantid2", "concated_embeddings_1", "concated_embeddings_2"]]
    ...
    if catboost:
        ...
        expanded_columns = pd.DataFrame(full_data['concated_embeddings_2'].tolist(), index=full_data.index)
        full_data = pd.concat([full_data.drop('concated_embeddings_2', axis=1), expanded_columns], axis=1)
        ...
        expanded_columns = pd.DataFrame(full_data['concated_embeddings_1'].tolist(), index=full_data.index)
        full_data = pd.concat([full_data.drop('concated_embeddings_1', axis=1), expanded_columns], axis=1)
